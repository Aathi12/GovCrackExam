...
try {
    const verStr = fs.readFileSync('data/phase27_verification.json', 'utf-8');
    ...
    let newQs = questions.filter(q => q.subtopic === 'Classification (Odd One Out)');
    assert(newQs.length === 16, "Exactly 16 Classification questions are in questions.json");
...
if (testsPassed) {
    console.log("All Phase 27 Verification tests passed.");
} else {
    process.exit(1);
}
